...
export default function ExcerciseContainer({email}: {email: string}){
    ...
    const myExercises  = useRef<Exercises[] | false>(exercises)
    ...
    <>
        <div className="flex flex-col bg-gray-200 bg-opacity-50 rounded-md mt-4 p-2">
            {myExercises.current === false ||  myExercises.current?.length === 0   ? <p>You currently have no exercises. Go ahead and create one!</p> : myExercises.current?.map((item: Exercises, key: number) => {
                return(                    
                <ExcerciseItem key={key} workout={item}/>
                ) 
            })}
        </div>
    </>)
